    ...
    stages{                                                                                                                                                                                
        ...
        stage('Integration Test') {
          steps {
            ...
            script {
              withDockerNetwork{ n ->
                docker.image("mariadb:10.10.2").withRun("-p 3306:3306 --network ${n} --hostname db -e MARIADB_PASSWORD='password' -e MARIADB_USER='user' -e MARIADB_DATABASE='db' -v tests/integration/init.sql:/docker-entrypoint-initdb.d/init.sql") { c ->
